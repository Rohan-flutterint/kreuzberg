...
internal sealed class OutputFormatJsonConverter : JsonConverter<OutputFormat>
{
    public override OutputFormat Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        ...
        return value switch
        {            "plain" => OutputFormat.Plain,            "markdown" => OutputFormat.Markdown,            "djot" => OutputFormat.Djot,            "html" => OutputFormat.Html,            "json" => OutputFormat.Json,            "structured" => OutputFormat.Structured,            "custom" => OutputFormat.Custom,            _ => throw new JsonException($"Unknown OutputFormat value: {value}")
        };
    }
    ...
    public override void Write(Utf8JsonWriter writer, OutputFormat value, JsonSerializerOptions options)
    {
        ...
        {            OutputFormat.Plain => "plain",            OutputFormat.Markdown => "markdown",            OutputFormat.Djot => "djot",            OutputFormat.Html => "html",            OutputFormat.Json => "json",            OutputFormat.Structured => "structured",            OutputFormat.Custom => "custom",            _ => throw new JsonException($"Unknown OutputFormat value: {value}")
        };
