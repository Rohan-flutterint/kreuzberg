...
public static class KreuzbergLib
{
    ...
    /// </summary>
    /// <exception cref="KreuzbergException">Propagates any LLM client / request error returned by
    /// `complete_text`.</exception>
    /// <param name="text"></param>
    /// <param name="llmConfig"></param>
    /// <param name="maxTokens">Optional.</param>
    public static async Task<string> SummarizeWithLlmAsync(string text, LlmConfig llmConfig, uint? maxTokens)
    {
        ArgumentNullException.ThrowIfNull(text);
        ...
        try
        {
            return await Task.Run(() =>
            {
                var nativeResult = NativeMethods.SummarizeWithLlm(
                text,
                llmConfigHandle,
                maxTokens ?? uint.MaxValue
                );
    ...
    /// </summary>
    /// <exception cref="KreuzbergException">Same as `extract_region_with_vlm`.</exception>
    /// <param name="imageBytes"></param>
    /// <param name="imageMime"></param>
    /// <param name="regionKind"></param>
    /// <param name="llmConfig"></param>
    /// <param name="customPrompt">Optional.</param>
    public static async Task<string> ExtractRegionWithVlmUsageAsync(byte[] imageBytes, string imageMime, RegionKind regionKind, LlmConfig llmConfig, string? customPrompt)
    {
        ArgumentNullException.ThrowIfNull(imageBytes);
        ArgumentNullException.ThrowIfNull(imageMime);
        ArgumentNullException.ThrowIfNull(regionKind);
        ...
        try
        {
            return await Task.Run(() =>
            {
                var nativeResult = NativeMethods.ExtractRegionWithVlmUsage(
                imageBytesHandle.AddrOfPinnedObject(),
                (UIntPtr)imageBytes.Length,
                imageMime,
                regionKindHandle,
    ...
    /// </summary>
    /// <param name="llm_config">— provider/model configuration.</param>
    ///
    /// <param name="prompt">— fully-rendered user prompt (no Jinja substitution performed).</param>
    ///
    /// <param name="schema_name">— name for the JSON schema (passed to providers that distinguish multiple structured outputs).</param>
    ///
    /// <param name="schema">— the JSON schema the LLM is required to obey.</param>
    ///
    /// <param name="source">— label used for the returned `LlmUsage` entry.</param>
    ///
    /// <exception cref="KreuzbergException">Returns an error if the LLM client cannot be constructed, the request fails,
    /// the response contains no content, or the response is not parseable JSON.</exception>
    /// <param name="llmConfig"></param>
    /// <param name="prompt"></param>
    /// <param name="schemaName"></param>
    /// <param name="schema"></param>
    /// <param name="source"></param>
    public static async Task<string> CompleteWithJsonSchemaAsync(LlmConfig llmConfig, string prompt, string schemaName, string schema, string source)
    {
        ArgumentNullException.ThrowIfNull(llmConfig);
        ...
        try
        {
            return await Task.Run(() =>
            {
                var nativeResult = NativeMethods.CompleteWithJsonSchema(
                llmConfigHandle,
                prompt,
                schemaName,
                schema,
    ...
    /// </summary>
    /// <exception cref="KreuzbergException">Returns an error if the LLM client cannot be constructed, the request fails,
    /// or the response does not contain assistant content.</exception>
    /// <param name="llmConfig"></param>
    /// <param name="prompt"></param>
    /// <param name="source"></param>
    public static async Task<string> CompleteTextAsync(LlmConfig llmConfig, string prompt, string source)
    {
        ...
        try
        {
            return await Task.Run(() =>
            {
                var nativeResult = NativeMethods.CompleteText(
                llmConfigHandle,
                prompt,
                source
                );
