...
public final class KreuzbergRs {
     ...
     *
     * {@code text} is the document content to summarise (already extracted by the
     * pipeline). {@code max_tokens} softly bounds the requested summary length in
     * natural-language tokens; {@code None} uses DEFAULT_MAX_TOKENS.
     *
     * Returns the summary string and the (optional) usage record.
     * {@literal @}throws KreuzbergRsException Propagates any LLM client / request error returned by
     * {@code complete_text}.
     */
    public static String summarizeWithLlm(
        final String text,
        ...
        try (var arena = Arena.ofShared()) {
            ...
            int cmaxTokens = (maxTokens == null) ? -1 : maxTokens;
            var resultPtr = (MemorySegment) NativeLib.KREUZBERG_SUMMARIZE_WITH_LLM.invoke(ctext, cllmConfig, cmaxTokens);
            if (!cllmConfig.equals(MemorySegment.NULL)) {
                NativeLib.KREUZBERG_LLM_CONFIG_FREE.invoke(cllmConfig);
            }
            if (resultPtr.equals(MemorySegment.NULL)) {
                checkLastError();                return null;            }
            long resultLen = (long) NativeLib.KREUZBERG_SUMMARIZE_WITH_LLM_LEN.invoke(ctext, cllmConfig, cmaxTokens);
            String str = readCString(resultPtr, resultLen);
            NativeLib.KREUZBERG_FREE_STRING.invoke(resultPtr);
            return str;
        } catch (Throwable e) {
    ...
    public static CompletableFuture<String> summarizeWithLlmAsync(final String text, final LlmConfig llmConfig, final int maxTokens) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                return summarizeWithLlm(text, llmConfig, maxTokens);
            } catch (Throwable e) {
                throw new CompletionException(e);
            }
        });
     ...
     * {@literal @}throws KreuzbergRsException Same as extract_region_with_vlm.
     */
    public static String extractRegionWithVlmUsage(
        final byte[] imageBytes,
        final String imageMime,
        final RegionKind regionKind,
        final LlmConfig llmConfig,
        ...
        try (var arena = Arena.ofShared()) {
            ...
            var ccustomPrompt = customPrompt != null ? arena.allocateFrom(customPrompt) : MemorySegment.NULL;
            var resultPtr = (MemorySegment) NativeLib.KREUZBERG_EXTRACT_REGION_WITH_VLM_USAGE.invoke(
                cimageBytes,
                cimageBytesLen,
                cimageMime,
                cregionKind,
            ...
            if (resultPtr.equals(MemorySegment.NULL)) {
                checkLastError();                return null;            }
            long resultLen = (long) NativeLib.KREUZBERG_EXTRACT_REGION_WITH_VLM_USAGE_LEN.invoke(
                cimageBytes,
                cimageBytesLen,
                cimageMime,
                cregionKind,
    ...
    public static CompletableFuture<String> extractRegionWithVlmUsageAsync(
        final byte[] imageBytes,
        final String imageMime,
        final RegionKind regionKind,
        final LlmConfig llmConfig,
        ...
        return CompletableFuture.supplyAsync(() -> {
            try {
                return extractRegionWithVlmUsage(imageBytes, imageMime, regionKind, llmConfig, customPrompt);
            } catch (Throwable e) {
                throw new CompletionException(e);
            }
        });
     ...
     * {@literal @}param llm_config — provider/model configuration.
     *
     * {@literal @}param prompt — fully-rendered user prompt (no Jinja substitution performed).
     *
     * {@literal @}param schema_name — name for the JSON schema (passed to providers that distinguish multiple structured outputs).
     *
     * {@literal @}param schema — the JSON schema the LLM is required to obey.
     *
     * {@literal @}param source — label used for the returned LlmUsage entry.
     *
     * {@literal @}throws KreuzbergRsException Returns an error if the LLM client cannot be constructed, the request fails,
     * the response contains no content, or the response is not parseable JSON.
     */
    public static String completeWithJsonSchema(
        final LlmConfig llmConfig,
        final String prompt,
        ...
        try (var arena = Arena.ofShared()) {
            ...
            var cprompt = arena.allocateFrom(prompt);
            var cschemaName = arena.allocateFrom(schemaName);
            var csource = arena.allocateFrom(source);
            var resultPtr = (MemorySegment) NativeLib.KREUZBERG_COMPLETE_WITH_JSON_SCHEMA.invoke(
                cllmConfig,
                cprompt,
                cschemaName,
                schema,
            ...
            if (resultPtr.equals(MemorySegment.NULL)) {
                checkLastError();                return null;            }
            long resultLen = (long) NativeLib.KREUZBERG_COMPLETE_WITH_JSON_SCHEMA_LEN.invoke(
                cllmConfig,
                cprompt,
                cschemaName,
                schema,
    ...
    public static CompletableFuture<String> completeWithJsonSchemaAsync(
        final LlmConfig llmConfig,
        final String prompt,
        final String schemaName,
        final Object schema,
        ...
        return CompletableFuture.supplyAsync(() -> {
            try {
                return completeWithJsonSchema(llmConfig, prompt, schemaName, schema, source);
            } catch (Throwable e) {
                throw new CompletionException(e);
            }
        });
     ...
     *
     * The {@code source} argument labels the LlmUsage entry that is returned so
     * callers can aggregate per-feature spend ({@code "translation"}, {@code "summarisation"},
     * etc.). The helper performs a single non-streaming chat completion request.
     * {@literal @}throws KreuzbergRsException Returns an error if the LLM client cannot be constructed, the request fails,
     * or the response does not contain assistant content.
     */
    public static String completeText(final LlmConfig llmConfig, final String prompt, final String source) throws KreuzbergRsException {
        try (var arena = Arena.ofShared()) {
            var cllmConfigJson = llmConfig != null ? MAPPER.writeValueAsString(llmConfig) : null;
            var cllmConfigJsonSeg = cllmConfigJson != null ? arena.allocateFrom(cllmConfigJson) : MemorySegment.NULL;
            var cllmConfig = cllmConfigJson != null
            ...
            var cprompt = arena.allocateFrom(prompt);
            var csource = arena.allocateFrom(source);
            var resultPtr = (MemorySegment) NativeLib.KREUZBERG_COMPLETE_TEXT.invoke(cllmConfig, cprompt, csource);
            if (!cllmConfig.equals(MemorySegment.NULL)) {
                NativeLib.KREUZBERG_LLM_CONFIG_FREE.invoke(cllmConfig);
            }
            if (resultPtr.equals(MemorySegment.NULL)) {
                checkLastError();                return null;            }
            long resultLen = (long) NativeLib.KREUZBERG_COMPLETE_TEXT_LEN.invoke(cllmConfig, cprompt, csource);
            String str = readCString(resultPtr, resultLen);
            NativeLib.KREUZBERG_FREE_STRING.invoke(resultPtr);
            return str;
        } catch (Throwable e) {
    ...
    public static CompletableFuture<String> completeTextAsync(final LlmConfig llmConfig, final String prompt, final String source) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                return completeText(llmConfig, prompt, source);
            } catch (Throwable e) {
                throw new CompletionException(e);
            }
        });
