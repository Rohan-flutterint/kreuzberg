...
// SummarizeWithLlm run abstractive summarisation against the configured LLM.
//
...
//
// Errors are returned when propagates any LLM client / request error returned by
// `complete_text`.
func SummarizeWithLlm(text string, llmConfig LlmConfig, maxTokens *uint32) (string, error) {
	...
	ptr := C.kreuzberg_summarize_with_llm(cText, cLlmConfig, cMaxTokens)
	if err := lastError(); err != nil {
		if ptr != nil {
			C.kreuzberg_free_string(ptr)
		}
...
// ExtractRegionWithVlmUsage same as [`extract_region_with_vlm`], but also returns the [`LlmUsage`] data captured
...
//
// Errors are returned when same as [`extract_region_with_vlm`].
func ExtractRegionWithVlmUsage(imageBytes []byte, imageMime string, regionKind RegionKind, llmConfig LlmConfig, customPrompt *string) (string, error) {
	...
	ptr := C.kreuzberg_extract_region_with_vlm_usage(cImageBytes, cImageBytesLen, cImageMime, cRegionKind, cLlmConfig, cCustomPrompt)
	if err := lastError(); err != nil {
		if ptr != nil {
			C.kreuzberg_free_string(ptr)
		}
...
// CompleteWithJSONSchema send a free-form prompt to the configured LLM with a JSON-schema response
...
//   - schema_name: — name for the JSON schema (passed to providers that distinguish multiple structured outputs).
//   - schema: — the JSON schema the LLM is required to obey.
//   - source: — label used for the returned [`LlmUsage`] entry.
//
// Errors are returned when returns an error if the LLM client cannot be constructed, the request fails,
// the response contains no content, or the response is not parseable JSON.
func CompleteWithJSONSchema(llmConfig LlmConfig, prompt string, schemaName string, schema json.RawMessage, source string) (string, error) {
	jsonBytescLlmConfig, err := json.Marshal(llmConfig)
	if err != nil {
		return "", fmt.Errorf("failed to marshal: %w", err)
	}
	...
	ptr := C.kreuzberg_complete_with_json_schema(cLlmConfig, cPrompt, cSchemaName, cSchema, cSource)
	if err := lastError(); err != nil {
		if ptr != nil {
			C.kreuzberg_free_string(ptr)
		}
...
// CompleteText send a single user prompt to the configured LLM and return the response text
...
func CompleteText(llmConfig LlmConfig, prompt string, source string) (string, error) {
	...
	ptr := C.kreuzberg_complete_text(cLlmConfig, cPrompt, cSource)
	if err := lastError(); err != nil {
		if ptr != nil {
			C.kreuzberg_free_string(ptr)
		}
