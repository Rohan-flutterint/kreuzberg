...
//!
//! ```text
//! cargo test -p kreuzberg --features "liter-llm,pdf,captioning,ocr" \
//!   --test captioning_smoke -- --nocapture --test-threads=1
//! ```

#![cfg(all(feature = "captioning", not(target_os = "windows")))]

use kreuzberg::core::config::{CaptioningConfig, ExtractionConfig, ImageExtractionConfig, LlmConfig};
...
async fn run_captioning_against_pdf(model: &str, api_key: String) {
    ...
    kreuzberg::plugins::processor::builtin::register_builtin().expect("register_builtin failed");

    let result = kreuzberg::extract_file(IMAGES_PDF, None, &config)
        .await
...
async fn captioning_post_processor_is_noop_without_images() {
    use kreuzberg::plugins::PostProcessor;
    use kreuzberg::plugins::processor::builtin::captioning::CaptioningProcessor;
