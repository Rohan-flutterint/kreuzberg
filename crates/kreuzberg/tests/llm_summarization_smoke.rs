...
//!
//! ```text
//! cargo test -p kreuzberg --features "summarization-llm,quality" --test llm_summarization_smoke -- --nocapture
//! ```
...
use kreuzberg::core::config::{ExtractionConfig, LlmConfig, SummarizationConfig};
use kreuzberg::plugins::PostProcessor;
use kreuzberg::plugins::processor::builtin::summarization::SummarizationProcessor;
...
async fn abstractive_summary_runs_against_real_provider() {
    ...
    let usage = result
        ...
        .iter()
        .find(|u| u.source == kreuzberg::text::summarization::llm::USAGE_SOURCE)
