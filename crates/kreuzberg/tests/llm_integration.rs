...
//!
//! ```text
//! cargo test -p kreuzberg --features "liter-llm,pdf" --test llm_integration -- --nocapture --test-threads=1
//! ```
//!
...
//! `ExtractionConfig`), matching how downstream callers (kreuzberg-cloud,
//! kreuzberg-py) invoke the engine.

#![cfg(feature = "liter-llm")]

use kreuzberg::core::config::{ExtractionConfig, LlmConfig, OcrConfig, StructuredExtractionConfig, VlmFallbackPolicy};
use serde_json::json;
...
async fn run_vlm_ocr(model: &str, api_key: String) {
    ...
    let result = kreuzberg::extract_file(HELLO_PNG, None, &config)
        .await
...
async fn run_structured(model: &str, api_key: String, strict: bool) {
    ...
    let result = kreuzberg::extract_file(MEMO_PDF, None, &config)
        .await
