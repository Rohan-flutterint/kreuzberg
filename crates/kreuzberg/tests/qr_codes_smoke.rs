...
impl kreuzberg::plugins::Plugin for SmokeQrProcessor {
    fn name(&self) -> &str {
        "qr-codes-smoke"
    }
...
impl PostProcessor for SmokeQrProcessor {
    async fn process(&self, result: &mut ExtractionResult, config: &ExtractionConfig) -> kreuzberg::Result<()> {
        ...
        for image in images.iter_mut() {
            let codes = kreuzberg::extractors::qr::detect_qr_codes(image.data.as_ref(), Some(image.format.as_ref()));
