//! FUNSD (Form Understanding in Noisy Scanned Documents) dataset loader.
...
/// Load FUNSD dataset fixtures from the given root directory.
///
/// The FUNSD dataset contains scanned form documents with entity and linking annotations.
/// Fixtures are organized by split (train/val/test).
///
/// # Arguments
///
/// * `root` - Root directory containing the FUNSD dataset
...
pub fn load(root: &Path, split: Split) -> Result<Vec<StructuredFixture>> {
    ...
    let dataset_root = root.join("FUNSD").join(split_name);
    if !dataset_root.exists() {
        return Err(DatasetError::NotFound(dataset_root.display().to_string()));
    }

    let manifest_path = root.join("manifests").join("funsd.toml");
    let manifest_content = fs::read_to_string(&manifest_path)
        .map_err(|e| DatasetError::Other(format!("Failed to read FUNSD manifest: {}", e)))?;

    let schema = load_funsd_schema()?;
...
/// Load the FUNSD JSON schema.
fn load_funsd_schema() -> Result<Value> {
    Ok(json!({
